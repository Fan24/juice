...
param = {
    "activityUrl": "https://pro.m.jd.com/mall/active/4GiwjZaSgiaCQHtWeghp9qa8vwZq/index.html",
    "prizeUrl": "https://api.m.jd.com/client.action"
}


def visit_activity(driver, userInfo):
    driver.get(param['activityUrl'])
    driver.find_element_by_id('m_1_3').find_element_by_xpath('./a').click()
    time.sleep(5)
    if driver.current_url.startswith('https://plogin.m.jd.com/user/login.action'):
        Common.jd_login(driver, userInfo, conf)
    if driver.current_url.startswith('https://pro.m.jd.com/mall/active/4GiwjZaSgiaCQHtWeghp9qa8vwZq/index.html'):
        return True
    return False
